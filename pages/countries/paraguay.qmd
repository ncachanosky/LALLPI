...
```{python}
#| echo: false
#| label: fig-paraguay-trend
#| fig-cap: "Paraguay: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "PRY")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-paraguay-breakdown
#| fig-cap: "Paraguay: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "PRY")
plt.show()
```

## Paraguay vs. the regional average

```{python}
#| echo: false
#| label: fig-paraguay-regional
#| fig-cap: "Paraguay vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "PRY")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "PRY")))
```
