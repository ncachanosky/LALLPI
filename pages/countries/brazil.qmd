...
```{python}
#| echo: false
#| label: fig-brazil-trend
#| fig-cap: "Brazil: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "BRA")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-brazil-breakdown
#| fig-cap: "Brazil: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "BRA")
plt.show()
```

## Brazil vs. the regional average

```{python}
#| echo: false
#| label: fig-brazil-regional
#| fig-cap: "Brazil vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "BRA")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "BRA")))
```
