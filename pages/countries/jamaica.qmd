...
```{python}
#| echo: false
#| label: fig-jamaica-trend
#| fig-cap: "Jamaica: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "JAM")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-jamaica-breakdown
#| fig-cap: "Jamaica: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "JAM")
plt.show()
```

## Jamaica vs. the regional average

```{python}
#| echo: false
#| label: fig-jamaica-regional
#| fig-cap: "Jamaica vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "JAM")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "JAM")))
```
