...
```{python}
#| echo: false
#| label: fig-bolivia-trend
#| fig-cap: "Bolivia: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "BOL")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-bolivia-breakdown
#| fig-cap: "Bolivia: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "BOL")
plt.show()
```

## Bolivia vs. the regional average

```{python}
#| echo: false
#| label: fig-bolivia-regional
#| fig-cap: "Bolivia vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "BOL")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "BOL")))
```
