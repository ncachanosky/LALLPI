...
```{python}
#| echo: false
#| label: fig-el-salvador-trend
#| fig-cap: "El Salvador: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "SLV")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-el-salvador-breakdown
#| fig-cap: "El Salvador: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "SLV")
plt.show()
```

## El Salvador vs. the regional average

```{python}
#| echo: false
#| label: fig-el-salvador-regional
#| fig-cap: "El Salvador vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "SLV")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "SLV")))
```
