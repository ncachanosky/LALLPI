...
```{python}
#| echo: false
#| label: fig-haiti-trend
#| fig-cap: "Haiti: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "HTI")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-haiti-breakdown
#| fig-cap: "Haiti: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "HTI")
plt.show()
```

## Haiti vs. the regional average

```{python}
#| echo: false
#| label: fig-haiti-regional
#| fig-cap: "Haiti vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "HTI")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "HTI")))
```
