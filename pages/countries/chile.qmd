...
```{python}
#| echo: false
#| label: fig-chile-trend
#| fig-cap: "Chile: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "CHL")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-chile-breakdown
#| fig-cap: "Chile: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "CHL")
plt.show()
```

## Chile vs. the regional average

```{python}
#| echo: false
#| label: fig-chile-regional
#| fig-cap: "Chile vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "CHL")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "CHL")))
```
