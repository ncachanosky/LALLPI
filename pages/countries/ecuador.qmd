...
```{python}
#| echo: false
#| label: fig-ecuador-trend
#| fig-cap: "Ecuador: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "ECU")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-ecuador-breakdown
#| fig-cap: "Ecuador: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "ECU")
plt.show()
```

## Ecuador vs. the regional average

```{python}
#| echo: false
#| label: fig-ecuador-regional
#| fig-cap: "Ecuador vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "ECU")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "ECU")))
```
