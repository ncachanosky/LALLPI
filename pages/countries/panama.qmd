...
```{python}
#| echo: false
#| label: fig-panama-trend
#| fig-cap: "Panama: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "PAN")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-panama-breakdown
#| fig-cap: "Panama: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "PAN")
plt.show()
```

## Panama vs. the regional average

```{python}
#| echo: false
#| label: fig-panama-regional
#| fig-cap: "Panama vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "PAN")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "PAN")))
```
