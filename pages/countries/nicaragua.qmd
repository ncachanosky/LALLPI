...
```{python}
#| echo: false
#| label: fig-nicaragua-trend
#| fig-cap: "Nicaragua: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "NIC")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-nicaragua-breakdown
#| fig-cap: "Nicaragua: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "NIC")
plt.show()
```

## Nicaragua vs. the regional average

```{python}
#| echo: false
#| label: fig-nicaragua-regional
#| fig-cap: "Nicaragua vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "NIC")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "NIC")))
```
