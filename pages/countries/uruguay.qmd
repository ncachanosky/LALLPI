...
```{python}
#| echo: false
#| label: fig-uruguay-trend
#| fig-cap: "Uruguay: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "URY")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-uruguay-breakdown
#| fig-cap: "Uruguay: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "URY")
plt.show()
```

## Uruguay vs. the regional average

```{python}
#| echo: false
#| label: fig-uruguay-regional
#| fig-cap: "Uruguay vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "URY")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "URY")))
```
