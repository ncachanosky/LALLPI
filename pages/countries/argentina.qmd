...
```{python}
#| echo: false
#| label: fig-argentina-trend
#| fig-cap: "Argentina: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "ARG")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-argentina-breakdown
#| fig-cap: "Argentina: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "ARG")
plt.show()
```

## Argentina vs. the regional average

```{python}
#| echo: false
#| label: fig-argentina-regional
#| fig-cap: "Argentina vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "ARG")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "ARG")))
```
