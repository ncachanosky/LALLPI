...
```{python}
#| echo: false
#| label: fig-honduras-trend
#| fig-cap: "Honduras: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "HND")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-honduras-breakdown
#| fig-cap: "Honduras: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "HND")
plt.show()
```

## Honduras vs. the regional average

```{python}
#| echo: false
#| label: fig-honduras-regional
#| fig-cap: "Honduras vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "HND")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "HND")))
```
