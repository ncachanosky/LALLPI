...
```{python}
#| echo: false
#| label: fig-peru-trend
#| fig-cap: "Peru: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "PER")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-peru-breakdown
#| fig-cap: "Peru: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "PER")
plt.show()
```

## Peru vs. the regional average

```{python}
#| echo: false
#| label: fig-peru-regional
#| fig-cap: "Peru vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "PER")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "PER")))
```
