...
```{python}
#| echo: false
#| label: fig-venezuela-trend
#| fig-cap: "Venezuela: POP, PEP, and PIP by year"

import matplotlib.pyplot as plt

fig = sh.build_country_trend_chart(df, "VEN")
plt.show()
```
...
```{python}
#| echo: false
#| label: fig-venezuela-breakdown
#| fig-cap: "Venezuela: sub-index components, latest year"

fig = sh.build_subindex_breakdown_chart(df, "VEN")
plt.show()
```

## Venezuela vs. the regional average

```{python}
#| echo: false
#| label: fig-venezuela-regional
#| fig-cap: "Venezuela vs. regional average POP"

fig = sh.build_regional_comparison_chart(df, "VEN")
plt.show()
```
...
display(HTML(sh.build_party_history_table_html(df, "VEN")))
```
